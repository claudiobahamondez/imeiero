...
// label
    ...
    public void trabajoConUnSoloImei(String imei){
        try{
            ...
            if(imei.length()==15){
                ...
                new MainActivity.UploadInformation().execute(imei, item, usuario);
            }
        ...
        try{
            ...
            if(imei.length()==15){
                ...
                if(txtIMEI.getText().toString().trim().length()==0){
                    ...
                    procesandoTarea(true);
                    new MainActivity.UploadInformationFORTWO().execute(imei1, imei2, item, usuario);
                }
            }
        }catch (NumberFormatException ex){
            alertaDeError("Pinchar IMEI válido");
    ...
    class UploadInformation extends AsyncTask<String, Void, String> {
        String records ="";
        String the_imei = ""; String the_sku =""; String the_name="";
        @Override
        protected String doInBackground(String... strings) {
            ...
            try{
                ...
                PreparedStatement statement2 = connection.prepareStatement("INSERT INTO IMEI_STOCK (IMEI, SKU, " +
                        "USUARIO, BODEGA) VALUES ('"+the_imei+"','"+the_sku+"','"+the_name+"','"+c.bodega+"') ");
                statement2.executeUpdate();


                PreparedStatement statement3 = connection.prepareStatement("UPDATE IMEI_STOCK SET " +
                        "FECHA_INVENTARIO=FECHA_MOD WHERE IMEI='"+the_imei+"' AND FECHA_INVENTARIO IS NULL AND FECHA_EMPAQUE IS NULL");
    ...
    class UploadInformationFORTWO extends AsyncTask<String, Void, String> {
        String records ="";
        String the_imei = ""; String the_imei2 = "";String the_sku =""; String the_name="";
        @Override
        protected String doInBackground(String... strings) {
            the_imei = strings[0];
            the_imei2 = strings[1];
            ...
            try{
                ...
                PreparedStatement statement2 = connection.prepareStatement("INSERT INTO IMEI_STOCK (IMEI, IMEI2, SKU, " +
                        "USUARIO, BODEGA) VALUES ('"+the_imei+"','"+the_imei2+"','"+the_sku+"','"+the_name+"','"+c.bodega+"') ");
                statement2.executeUpdate();


                PreparedStatement statement3 = connection.prepareStatement("UPDATE IMEI_STOCK SET " +
                        "FECHA_INVENTARIO=FECHA_MOD WHERE IMEI='"+the_imei+"' AND IMEI2='"+the_imei2+"' AND FECHA_INVENTARIO IS NULL AND FECHA_EMPAQUE IS NULL");
                statement3.executeUpdate();
        ...
        protected void onPostExecute(String aVoid) {
            ...
            procesandoTarea(false);
            if(records.equals("OK")){
                exito(the_imei, the_imei2, the_sku);
            }
            if(records.equals("PROB")){
                alertaDeError("Problemas de conexion");
                procesandoTarea(false);
    ...
    public void realizarProceso(String imei, String sku, String nombre){
        new MainActivity.UploadInformation().execute(imei,sku,nombre);
    }
    ...
    public void exito(String imei, String imei2, String sku){
        ...
        txtIMEITWO.setVisibility(View.INVISIBLE);
        lblIMEITWO.setVisibility(View.INVISIBLE);
        txtRESULT.setText("IMEI: "+imei+"  , IMEI 2: "+imei2+" , SKU: "+sku);
        procesandoTarea(false);
    }
    ...
    public void preguntarCuantosImeiTieneElItem(String item){
        ...
        dialogo1.setPositiveButton("1", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogo1, int id) {
                procesandoTarea(true);
                new MainActivity.UpdateImeiQuantity().execute(item,"1",usuario);
            }
        });
        dialogo1.setNegativeButton("2", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogo1, int id) {
                procesandoTarea(true);
                new MainActivity.UpdateImeiQuantity().execute(item,"2",usuario);
            }
        });
        dialogo1.show();
    }
